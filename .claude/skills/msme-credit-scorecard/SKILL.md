...
Single sources are inputs; **fused cross-source signals are the product** — harder to fake than any one source. Full rubric-scored catalog (34 evaluated, 8 core + 18 enrichment + 8 rejected with reasons) and the 13-composite catalog: `internal/appendix-a-data-source-catalog.md`. Highlights:
...
- **GST** (goldmine): filing regularity, turnover trend/seasonality, credit-note intensity, concentration, ITC ratios; GST-vs-bank divergence is the signature red flag. GSTN-stable feature backlog: `internal/cag-gst-feature-analysis.md` (also lists obsolete indicators GSTN now auto-enforces — exclude them).
