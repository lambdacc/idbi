...
r1[0].markdown(kpi("Financial Health Score", f"{hc.composite_score:.0f}<small>/100</small>",
               f"CMR-style grade {hc.grade}/10", score_class(hc.composite_score)), unsafe_allow_html=True)
r1[1].markdown(kpi("Risk category", out["risk_category"],
               f"Model PD {out['pd']:.1%} · score {out['credit_score_300_900']}/900",
...
r2[1].markdown(kpi("Data confidence", out["confidence_band"],
               ...
               unsafe_allow_html=True)
r2[2].markdown(kpi("Peer segment", hc.peer_segment or "—", "K-Means (descriptive only)"),
               unsafe_allow_html=True)
