"""Track 03 · Run Assessment — scenario picker + kick off a run (multi-track D11
deep link `track03`; was the single-track `Home.py`).
...
picks a business and kicks off a run — all computation lives in backend/ml. The
platform brandmark hero now lives on the Overview page; this page keeps a normal
page header.
"""
...
def render() -> None:
    ...
    if go:
        ...
        if staged:
            st.switch_page(tracks.get_page("t03.pipeline"))
        else:
            st.switch_page(tracks.get_page("t03.health_card"))
